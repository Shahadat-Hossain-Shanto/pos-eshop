...
class PurchaseVoucherController extends Controller
{
    public function index(){
        ...
        $data = $cashInHand->concat($banks)->concat($suppliersX);
        // Log::info($data);
        ...
        return view('purchase-voucher/purchase-voucher', ['suppliers' => $suppliers, 'expenseAccounts' => $expenseAccounts,
                'datas' => $data]);
    }
    ...
    {
        return view('purchase-voucher/purchase-voucher-report');
    }
    public function show(Request $request, $transaction_id, $startdate, $enddate, $transaction_type)
    {
        if($transaction_id==0)
        {
            $purchase = Transaction::whereBetween('transaction_date', [$startdate, $enddate])
            ->where('transaction_type', '=', $transaction_type)
            ->orderBy('created_at', 'desc')
            ->get();
        }
        else{
            $purchase = Transaction::where('transaction_id', '=', $transaction_id)
            ->where('transaction_type', '=', $transaction_type)
            ->orderBy('created_at', 'desc')
            ->get();
        }
