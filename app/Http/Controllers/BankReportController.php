...
class BankReportController extends Controller
{
    public function index()
    {
        ...
        return view('report/bank-report', ['banks' => $banks]);
    }

    public function onLoad(Request $request)
    {
        if ($request->filled('employee') && $request->filled('enddate') && $request->filled('startdate')) {
    ...
    public function showReport(Request $request)
    {
        ...
        if (($request->input('employee') == 'option_select') && ($request->input('enddate') == NULL) && ($request->input('startdate') == NULL)) {
            ...
            $datas = DB::table("orders")
                ->select(DB::raw("SUM(total) as t"), DB::raw("SUM(totalDiscount) as td"), "salesBy_id")
        ...
        } else if ($request->filled('employee') && $request->filled('enddate') && $request->filled('startdate')) {
