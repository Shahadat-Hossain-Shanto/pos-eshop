                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="card card-primary">
                            <div class="card-header">
                                <h5 class="m-0"><strong><i class="fas fa-file-contract"></i>Supplier Payment Report</strong></h5>
                            </div>
                            <div class="card-body">
                                <div id="form_div">
                                    <form id="supplierPaymentReportForm" >
                                        <div class="form-row">
                                ...
                                <div id="tablePart" >
                                    <hr>
                                    <h5 class="pb-3"><strong><u>Supplier Payment History</u></strong></h5>
                                    {{-- <h6 class='float-right'><strong>Balance : </strong> &nbsp;&nbsp;&nbsp;<span id='debit'></span>Dr &nbsp;&nbsp;&nbsp;<span id='credit'></span>Cr &nbsp;&nbsp;&nbsp;</h6> --}}
                                    <table id="supplier_payment_table" class="display" style="100%!important">
                                        <thead>
                                            <tr>
                                                <th>#</th>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#supplierPaymentReportForm').on('submit', function(e) {
                var transaction_type = 'Supplier Payment';
               ...
               if (transaction_id !=''|| startdate.length != 0 && enddate.length != 0) {
                ...
                $.ajax({
                    type: "get",
                    url: "/supplier-payment-report/"+transaction_id+"/"+startdate+"/"+enddate+"/"+transaction_type,

                    success: function(response) {
                        // console.log(response.supplier_payments);
                        var totalDebit=0;
                        var totalCredit=0;
                        response.supplier_payments.forEach(supplier_payment => {
                            totalCredit = totalCredit+supplier_payment.credit;
                            totalDebit = totalDebit+supplier_payment.debit;
                        });
                        $('#debit').text(totalDebit.toFixed(2));
                        $('#credit').text(totalCredit.toFixed(2));
                        i=1;
                        $('#tablePart').show();
                        onChangeDataTable(response.supplier_payments,i)
...
$('#supplier_payment_table').DataTable().clear().destroy()

var t = $('#supplier_payment_table').DataTable({
