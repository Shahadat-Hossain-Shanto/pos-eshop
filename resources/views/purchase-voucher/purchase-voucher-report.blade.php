                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="card card-primary">
                            <div class="card-header">
                                <h5 class="m-0"><strong><i class="fas fa-file-contract"></i>Purchase Voucher Report</strong></h5>
                            </div>
                            <div class="card-body">
                                <div id="form_div">
                                    <form id="purchaseVoucherReportForm" >
                                        <div class="form-row">
                                ...
                                <div id="tablePart" >
                                    <hr>
                                    <h5 class="pb-3"><strong><u>Purchase Voucher History</u></strong></h5>
                                    {{-- <h6 class='float-right'><strong>Balance : </strong> &nbsp;&nbsp;&nbsp;<span id='debit'></span>Dr &nbsp;&nbsp;&nbsp;<span id='credit'></span>Cr &nbsp;&nbsp;&nbsp;</h6> --}}
                                    <table id="purchase_voucher_table" class="display" style="100%!important">
                                        <thead>
                                            <tr>
                                                <th>#</th>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#purchaseVoucherReportForm').on('submit', function(e) {
                var transaction_type = 'Purchase Voucher';
               ...
               if (transaction_id !=''|| startdate.length != 0 && enddate.length != 0) {
                ...
                $.ajax({
                    type: "get",
                    url: "/purchase-voucher-report/"+transaction_id+"/"+startdate+"/"+enddate+"/"+transaction_type,

                    success: function(response) {
                        // console.log(response.purchase);
                        var totalDebit=0;
                        var totalCredit=0;
                        response.purchase.forEach(product => {
                            totalCredit = totalCredit+product.credit;
                            totalDebit = totalDebit+product.debit;
                        });
                        $('#debit').text(totalDebit.toFixed(2));
                        $('#credit').text(totalCredit.toFixed(2));
                        i=1;
                        $('#tablePart').show();
                        onChangeDataTable(response.purchase,i)
...
$('#purchase_voucher_table').DataTable().clear().destroy()

var t = $('#purchase_voucher_table').DataTable({
